            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div x-data="{ search: '', statusFilter: '', openCustomer: null }" class="bg-white shadow-lg rounded-lg p-4">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <input type="text" x-model="search" placeholder="Search by customer or product..."
                            class="border p-2 rounded-lg w-1/3 focus:ring focus:ring-green-300">
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="/admin/orders/rejected" class="bg-red-500 hover:bg-red-800 hover:scale-105 transition-transform text-white p-2 rounded-lg px-3">Rejected</a>
                        </div>
                    </div>
                            ...
                            <div class="border rounded-lg overflow-hidden" x-data="{
                                        ...
                                        if (result.isConfirmed) {
                                            Promise.all(
                                                ...
                                                if (results.every(result => result.success)) {
                                                    Swal.fire('Deleted!', 'Selected orders deleted.', 'success')
                                                        .then(() => window.location.reload());
                                                } else {
                                                    Swal.fire('Error!', 'Some orders could not be deleted.', 'error');
                                                }
                                            }).catch(error => {
                                ...
                                markReject() {
                                    ...
                                    Swal.fire({
                                        ...
                                        text: 'Are you sure you want to reject the selected orders?',
                                        icon: 'warning',
                                        showCancelButton: true,
                                        confirmButtonColor: '#3085d6',
                                        cancelButtonColor: '#d33',
                                        ...
                                        if (result.isConfirmed) {
                                            Promise.all(
                                                ...
                                                if (results.every(result => result.success)) {
                                                    Swal.fire('Updated!', 'Selected orders marked as rejected.', 'success')
                                                        .then(() => window.location.reload());
                                                } else {
                                                    Swal.fire('Error!', 'Some orders could not be updated.', 'error');
                                                }
                                            }).catch(error => {
                                                console.error('Error:', error);
                                                Swal.fire('Oops!', 'Something went wrong.', 'error');
                                ...
                                <button
                                    @click="openCustomer === {{ $user_id }} ? openCustomer = null : openCustomer = {{ $user_id }}"
                                    class="w-full bg-gray-200 px-4 py-2 text-left font-semibold flex justify-between items-center">
                                    <span>{{ $userOrders->first()->user->firstname }}
                                        {{ $userOrders->first()->user->lastname }}</span>
                                    <span x-show="openCustomer === {{ $user_id }}">&#9650;</span>
                                    <span x-show="openCustomer !== {{ $user_id }}">&#9660;</span>
                                </button>
                                <div x-show="openCustomer === {{ $user_id }}" x-collapse class="p-4 space-y-4">
                                    @foreach ($userOrders as $order)
                                        <div class="shadow rounded-lg flex justify-between bg-white p-1 px-2"
                                            x-show="
...
&&
                                            ...
                                            <div>
                                                <h3 class="font-semibold text-gray-900">{{ $order->product->name }}
                                                </h3>
                                                <p class="text-gray-700">Quantity: {{ $order->quantity }}</p>
                                                <p class="text-gray-700">
                                                    Status:
                                                    <span
                                                        class="px-2 rounded-lg text-white text-sm font-semibold {{ $order->status == 'pending' ? 'bg-yellow-500' : 'bg-red-500' }}">
                                    ...
                                    <!-- Single Mark as Completed button for this customer group -->
